...
void CommandQueue::PopulateCommandList(Microsoft::WRL::ComPtr<ID3D12Resource> inRenderTarget, int inFrameIndex, Microsoft::WRL::ComPtr<ID3D12DescriptorHeap> inRtvDescriptorHeap, int inRtvDescriptorSize)
{
...
void CommandQueue::WaitForPreviousFrame()
{
    ...
    //if (fence[inFrameIndex]->GetCompletedValue() < fenceValue[inFrameIndex])
    //{
    //     we have the fence create an event which is signaled once the fence's current value is "fenceValue"
    //    hr = fence[inFrameIndex]->SetEventOnCompletion(fenceValue[inFrameIndex], mFenceEvent);
    //    if (FAILED(hr))
    //    {
    //       
    //    }
